...
export const translateToSpanish = async (text: string): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Traduce la siguiente descripción de un videojuego al español. Debe sonar profesional, atractivo y natural. Solo devuelve el texto traducido sin introducciones:\n\n"${text}"`,
    });
